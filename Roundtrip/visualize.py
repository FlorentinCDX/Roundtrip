...
class RTMload:
    def __init__(self, x_dim, y_dim, n_particules=1000) -> None:
        self.g_net = model.Generator_(latent_dim = x_dim, out_shape= y_dim)   
        self.h_net = model.Generator_(latent_dim= y_dim, out_shape = x_dim)
        self.dx_net = model.Discriminator_(inp_shape = x_dim)
        self.dy_net = model.Discriminator_(inp_shape = y_dim)
        self.xs = sampler.Gaussian_sampler(mean=np.zeros(x_dim),sd=1.0, batch_size = n_particules)
        self.x_dim = x_dim
        self.y_dim = y_dim
...
class RTModel(RTMload):
    def __init__(self, data = 'indep_gmm', x_dim = 2, y_dim = 2):
        ...
        self.data = data
        if data == 'indep_gmm':
            self.ys = sampler.GMM_indep_sampler(sd=0.1, dim=y_dim, n_components=3, bound=1, batch_size=self.n_particules)
            ...
            self.ys = sampler.GMM_sampler(batch_size=self.n_particules,mean=mean,cov=cov)
        
        self.model = RoundtripModel(self.g_net, self.h_net, self.dx_net, self.dy_net, self.xs, data, self.ys, 10, 10)
    ...
    def visualize(self):
        ...
        fig.savefig('images/{}.png'.format(self.data))
        plt.close(fig)
